...
class list
{
  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
      auto itrPos = begin();
      for(int i=0;i<pos;i++) itrPos++;
      
      int n=0;
      for(int i=0;i<selected.size();i++){
        auto itsel = begin();
        int selIdx = selected[i]-n; //ถอย index หลังจากลบไปแล้ว

        for(int j=0;j<selIdx;j++) itsel++;
        insert(itrPos,*itsel);
        //std::cout << *itsel << std::endl;
        erase(itsel); //ลบที่เดิม

        if(pos > selected[i]) n++;
        //std::cout << selIdx << " " << n << std::endl;
      }
    }
